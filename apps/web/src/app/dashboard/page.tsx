...
import { 
  ...
  ArrowRight, Activity,
  Calendar, DollarSign, Users, Star, Crown
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const quickActions = [
    { icon: <Image className="w-5 h-5" />, label: 'Créer une image', href: '/dashboard/image/new', color: 'from-orange-500 to-pink-500', bg: 'bg-orange-500/10' },
    { icon: <Video className="w-5 h-5" />, label: 'Créer une vidéo', href: '/dashboard/video/new', color: 'from-pink-500 to-purple-500', bg: 'bg-pink-500/10' },
    { icon: <Sparkles className="w-5 h-5" />, label: 'Générer un site', href: '/dashboard/website/new', color: 'from-purple-500 to-indigo-500', bg: 'bg-purple-500/10' },
    { icon: <ShoppingBag className="w-5 h-5" />, label: 'Marketplace', href: '/dashboard/marketplace', color: 'from-indigo-500 to-blue-500', bg: 'bg-indigo-500/10' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="bg-gradient-to-r from-orange-500/20 to-pink-500/20 backdrop-blur-sm border border-orange-500/20 rounded-2xl p-6">
            ...
            <p className="text-3xl font-bold text-white mt-1">{user?.credits || 10}</p>
            <Link href="/dashboard/billing" className="text-xs text-orange-500 hover:underline mt-2 inline-block">
              Recharger →
            </Link>
          </motion.div>
        </motion.div>
